...
class Paciente extends CI_Controller {
    ...
    public function buscarPaciente(){
        ...
        if(isset($datos['txtBuscar'])){
            ...
            if($result!=FALSE){
                redirect('paciente/findPaciente/'.$txtBuscar.'/0');
            }
            else{            
                $data['contenido'] = "paciente/index";            
                $data['id'] = $txtBuscar;    
                $this->session->set_flashdata('errorFind', 'El Paciente No Existe');
                $data['error'] = $this->session->flashdata('errorFind');
                $this->load->view("plantilla",$data);            
            }
        }        
    }
    
    public function findPaciente($id,$msg){
        ...
        $resultHistorias = $this->Model_HistoriaClinica->buscarHistorias($id_paciente); //traer historias clinicas del paciente                
        $data['historias'] = $resultHistorias;
        if($msg=='1'){ 
            $this->session->set_flashdata('exitoMod', 'El Nuevo paciente se registró con exito');
            $data['error'] = $this->session->flashdata('exitoMod');
        }
        else if($msg=='2'){
            $this->session->set_flashdata('exitoMod', 'Los datos del paciente se actualizaron correctamente');
            $data['error'] = $this->session->flashdata('exitoMod');            
        }
        else if($msg=='3'){
            $this->session->set_flashdata('exitoMod', 'La Nueva Historia Clinica se registró con exito');
            $data['msgHistoria'] = $this->session->flashdata('exitoMod');
        }
        else if($msg=='4'){
            $this->session->set_flashdata('exitoMod', 'Los datos de la historia clinica se actualizaron correctamente');
            $data['msgHistoria'] = $this->session->flashdata('exitoMod');
        }
    ...
    public function insert(){
        ...
        if(isset($datos) && isset($datos['txtIdentificacion'])){
            ...
            if($txtIdentificacion!=NULL){
                ...
                $optTipo = $datos['optTipo'];                               $txtLabios = $datos['txtLabios'];
                $txtDescTipoPaciente = $datos['txtDescTipoPaciente'];       $txtGs = $datos['txtGs'];
                $fechaIngreso = $datos['fechaIngreso'];                     $txtLengua = $datos['txtLengua'];
                $txtApellido1 = $datos['txtApellido1'];                     $txtCarrillos = $datos['txtCarrillos'];
                $txtApellido2 = $datos['txtApellido2'];                     $txtPaladar = $datos['txtPaladar'];
                $txtNombres = $datos['txtNombres'];                         $txtMo = $datos['txtMo'];
                $txtPb = $datos['txtPb'];                                   $txtObservacionAnamnesis = $datos['txtObservacionAnamnesis'];
                $fechaNacimiento = $datos['fechaNacimiento'];               $txtMaxilares = $datos['txtMaxilares'];
                $txtEdad = $datos['txtEdad'];                               $txtDientes = $datos['txtDientes'];
                $txtDireccion = $datos['txtDireccion'];                     $txtCariados = $datos['txtCariados'];
                $txtDepartamento = $datos['txtDepartamento'];               $txtProtesis = $datos['txtProtesis'];
                $txtMunicipio = $datos['txtMunicipio'];                     $txtDescripProtesis = $datos['txtDescripProtesis'];
                $txtEmail = $datos['txtEmail'];                             $txtObturados = $datos['txtObturados'];
                $txtOcupacion = $datos['txtOcupacion'];                     $txtPerdidos = $datos['txtPerdidos'];
                $txtDireccionOcupacion = $datos['txtDireccionOcupacion'];   $txtSanos = $datos['txtSanos'];
                $txtAcudiente = $datos['txtAcudiente'];                     $txtEp = $datos['txtEp'];
                $txtParentesco = $datos['txtParentesco'];                   $txtTd = $datos['txtTd'];
                $txtTelefonoAcudiente = $datos['txtTelefonoAcudiente'];     $txtPeriodonto = $datos['txtPeriodonto'];
                $optSh = $datos['optSh'];                                   $txtOclusion = $datos['txtOclusion'];
                $txtDescRedes = $datos['txtDescRedes'];                     $txtObservacionesPaciente = $datos['txtObservacionesPaciente'];
                $txtMotivoConsulta = $datos['txtMotivoConsulta'];           $optDiabetes = $datos['optDiabetes'];            
                $optTm = $datos['optTm'];                                   $optSe = $datos['optSe'];                              
                $optSr = $datos['optSr'];                                   $optHepatitis = $datos['optHepatitis'];
                $optPv = $datos['optPv'];                                   $optOp = $datos['optOp'];
                $optSi = $datos['optSi'];                                   $optSinusitis = $datos['optSinusitis'];
                $optCv = $datos['optCv'];                                   $optFr = $datos['optFr'];
                ...
                if(!empty($datos['optRedes'])) $optRedes = $datos['optRedes'];
                if(!empty($datos['optHo'])) $optHo = $datos['optHo'];
                if(!empty($datos['optGr'])) $optGr = $datos['optGr'];
                if(!empty($datos['optSd'])) $optSd = $datos['optSd'];
                if(!empty($datos['optFc'])) $optFc = $datos['optFc'];
                if(!empty($datos['optPigmentacion'])) $optPigmentacion = $datos['optPigmentacion'];
                ...
                try{
                    ...
                    redirect('paciente/findPaciente/'.$txtIdentificacion.'/1');
        ...
        if(isset($datos) && isset($datos['optTipo'])){            
            if($identificacion!=NULL){
                ...
                try{
                    ...
                    redirect('paciente/findPaciente/'.$identificacion.'/2');                    
